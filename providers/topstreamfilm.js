...
const SITE = 'https://www.topstreamfilm.live';
...
async function list(path = '/') {
  ...
  $('.post, .card, .thumb, article.post, .ml-item').each((i, el) => {
    const el$ = $(el);
    const link = el$.find('a').attr('href');
    const title = el$.find('h2, .title, .entry-title').text().trim() || el$.find('img').attr('alt');
...
async function details(pageUrl) {
  ...
  const desc = $('.entry-content, .description').text().trim();
  const poster = $('meta[property="og:image"]').attr('content') || $('img.poster').attr('src');

  const links = [];
  $('.mirrors a, .player-links a, a[href*="embed"], a[href*="player"]').each((i, el) => {
    const h = $(el).attr('href');
    if (h) links.push(new URL(h, pageUrl).href);
  });

  return { title, description: desc, poster, links };
}
